...
class OptimizedRSSProcessor:
    ...
    def _parse_rss_feed(self, url: str, source_info: Dict) -> List[NewsArticle]:
        ...
        try:
            # Check cache first
            cached_articles = self.cache.get(url, max_age=1)  # Temporarily set max_age to 1 second for testing
            if cached_articles:
                logger.info(f"Cache hit for {source_info['name']}. Returning cached articles.") # Added for debugging
                return cached_articles
            
            logger.info(f"Cache miss for {source_info['name']}. Fetching new articles.") # Added for debugging
            ...
            fetch_time = time.time() - start_time
            logger.info(f"Fetched {len(articles)} articles from {source_info['name']} in {fetch_time:.2f}s. First article: {articles[0].title if articles else 'N/A'}") # Added for debugging
