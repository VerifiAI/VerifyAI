    ...
    try:
        ...
        if not source or source not in rss_feeds:
            mock_articles = [
                ...
                {
                    'title': 'Global Economic Update: Markets Show Stability',
                    'link': 'https://example.com/news/2', 
                    'description': 'Financial markets demonstrate resilience amid global challenges.',
                    'published': datetime.now().isoformat(),
                    'source': 'MOCK'
                },
                {
                    ...
                    'description': 'Scientists publish new data on environmental impact studies.',
                    'published': datetime.now().isoformat(),
                    'source': 'MOCK'
                }
            ]
            ...
            return jsonify({
                'status': 'success',
                'message': 'Mock news data for testing',
                'source': 'MOCK',
                'data': mock_articles,
...
def get_extended_metrics():
    ...
    try:
        ...
        for record in recent_predictions:
            try:
                # Assume we have ground truth labels (in real scenario, this would come from validation data)
                # For demo purposes, we'll simulate ground truth based on confidence patterns
                confidence = float(record.get('confidence', 0.5))
                prediction = record.get('prediction', 'real')
                
                # Simulate ground truth (in production, this should come from actual labels)
                # Higher confidence fake predictions are more likely to be actually fake
                if prediction == 'fake' and confidence > 0.8:
                    true_label = 1  # Actually fake
                elif prediction == 'fake' and confidence < 0.6:
                    true_label = 0  # Actually real (false positive)
                elif prediction == 'real' and confidence > 0.8:
                    true_label = 0  # Actually real
                else:
