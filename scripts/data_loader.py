...
class FakeNewsDataLoader:
    ...
    def load_parquet_files(self) -> Dict[str, pd.DataFrame]:
        ...
        for split, filename in parquet_files.items():
            try:
                ...
                if not file_path.exists():
                    logger.warning(f"Creating mock data for missing file: {filename}")
                    # Create mock data if file doesn't exist
                    mock_data = self._create_mock_parquet_data(split)
                    loaded_data[split] = mock_data
                    continue
                ...
                logger.info(f"Creating fallback mock data for {split}")
                loaded_data[split] = self._create_mock_parquet_data(split)
    ...
    def _create_mock_parquet_data(self, split: str) -> pd.DataFrame:
        """
        Create mock Parquet data for testing purposes.
        ...
        """
        sizes = {'train': 1000, 'val': 200, 'test': 300}
        size = sizes.get(split, 100)
        
        np.random.seed(42)  # For reproducibility
        
        mock_data = {
            'id': range(size),
            'title': [f"Sample news title {i}" for i in range(size)],
            'text': [f"Sample news content {i} with some text data" for i in range(size)],
            'label': np.random.randint(0, 2, size),  # Binary classification
            'image_path': [f"image_{i}.jpg" for i in range(size)],
            'features': [np.random.randn(300).tolist() for _ in range(size)]
        }
        
        df = pd.DataFrame(mock_data)
        logger.info(f"Created mock {split} data: {df.shape}")
        return df
    ...
    def load_pickle_files(self) -> Dict[str, Any]:
        ...
        for key, filename in pickle_files.items():
            try:
                ...
                if not file_path.exists():
                    logger.warning(f"Creating mock pickle data for missing file: {filename}")
                    loaded_pickle[key] = self._create_mock_pickle_data(key)
                    continue
    ...
    def _create_mock_pickle_data(self, key: str) -> pd.DataFrame:
        """
        Create mock pickle data for testing.
        
        Args:
            key (str): Data key identifier
            
        Returns:
            pd.DataFrame: Mock pickle dataframe
        """
        size = 500 if 'train' in key else 200
        np.random.seed(42)
        
        mock_data = {
            'id': range(size),
            'text_features': [np.random.randn(300).tolist() for _ in range(size)],
            'image_features': [np.random.randn(300).tolist() for _ in range(size)],
            'combined_features': [np.random.randn(600).tolist() for _ in range(size)],
            'label': np.random.randint(0, 2, size)
        }
        
        df = pd.DataFrame(mock_data)
        logger.info(f"Created mock pickle data for {key}: {df.shape}")
        return df
